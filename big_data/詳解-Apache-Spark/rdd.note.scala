//Sparkはイミュータブルな分散コレクションであるRDDを作成し、RDDの操作を通じて分散処理を行います。
//そのため、RDDはSparkの分散処理の根幹を支える要素とも言えるでしょう。RDDを正しく理解することがSparkの分散処理のパワーを効果的に活用する近道です。
//オブジェクト指向プログラミングにおいて、イミュータブル(immutable)なオブジェクトとは、作成後にその状態を変えることのできないオブジェクトのことである。

// RDD（Resilient Distributed Dataset）
// はイミュータブルなデータ要素の分散コレクションです。SparkではこのRDDに対するデータ
// 処理を行うことで、分散処理を実現しています。RDDはパーティションと呼ばれる単位に分割
// され、Sparkクラスタの複数のノードに分散して配置されます。Sparkではパーティション単位
// で処理が行われるため、大量のデータでも複数のノードで分散して処理できます。
